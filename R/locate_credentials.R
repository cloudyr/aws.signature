...
function(key = NULL, 
    ...
    if (is.null(region)) {
        ...
        if (isTRUE(verbose)) {
            message("Using user-supplied value for AWS Region")
        }
    }
...
find_value <- function(value, file, profile, verbose = FALSE, fail = FALSE) {
    ...
    if (is.null(value) || value == "") {
        ...
        if (!missing(file)) {
            credentials <- try(read_credentials(file)[[profile]], silent = TRUE)
        } else {
            credentials <- try(read_credentials(file.path(".aws", "credentials"))[[profile]], silent = TRUE)
        }
        if (inherits(credentials, "try-error")) {
            credentials <- try(read_credentials()[[profile]], silent = TRUE)
            if (inherits(credentials, "try-error")) {
                ec2role <- try(get_ec2_role(), silent = TRUE)
